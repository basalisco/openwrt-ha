...
# ============================================================
# Funzione: applica o disattiva una regola firewall
# ============================================================
apply_firewall() {
    local rule_name="\$1"
    local action="\$2"
    ...
    if [ -n "\$INDEX" ]; then
        ...
        if [ "\$action" = "1" ]; then
            ...
            if [ -n "\$MAC" ]; then
                IP=\$(cat /etc/config/dhcp.leases | grep -i "\$MAC" | awk '{print \$3}')   #inserisi il percorso corretto al file leases
                [ -n "\$IP" ] && conntrack -D -s "\$IP" 2>/dev/null
            fi
        fi

        mosquitto_pub -h "\$MQTT_HOST" -p "\$MQTT_PORT" -u "\$MQTT_USER" -P "\$MQTT_PASS" \\
            -t "\$PREFIX/switch/\$NODE_ID/\$rule_name/state" -m "\$action" -r
    fi
}

# ============================================================
# Funzione: pulizia regole orfane dal topic MQTT
# ============================================================
cleanup_orphans() {
...
# ============================================================
# Funzione: pubblica discovery MQTT e stato regole firewall
# ============================================================
discovery_and_state() {
    cleanup_orphans
    ...
    mosquitto_pub -h "\$MQTT_HOST" -p "\$MQTT_PORT" -u "\$MQTT_USER" -P "\$MQTT_PASS" \\
        -t "\$PREFIX/button/\$NODE_ID/refresh/config" -m "\$REFRESH_PAYLOAD" -r
    ...
    while true; do
        ...
        mosquitto_pub -h "\$MQTT_HOST" -p "\$MQTT_PORT" -u "\$MQTT_USER" -P "\$MQTT_PASS" \\
            -t "\$BASE_TOPIC/config" -m "\$PAYLOAD" -r

        ENABLED=\$(uci get firewall.@rule[\$i].enabled 2>/dev/null || echo "1")
        mosquitto_pub -h "\$MQTT_HOST" -p "\$MQTT_PORT" -u "\$MQTT_USER" -P "\$MQTT_PASS" \\
            -t "\$BASE_TOPIC/state" -m "\$ENABLED" -r
...
# ============================================================
# Funzione: ascolta comandi MQTT
# ============================================================
listen() {
    mosquitto_sub -h "\$MQTT_HOST" -p "\$MQTT_PORT" -u "\$MQTT_USER" -P "\$MQTT_PASS" \\
        -t "\$PREFIX/+/\$NODE_ID/+/set" -v | while read -r line; do
